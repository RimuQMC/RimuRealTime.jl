...
- `scaling_strategy = NoScaling()`: Which [`ScalingStrategy`](@ref) to use to control the
  walker number.
"""
Base.@kwdef struct CFCIQMC{TS<:TimeStepStrategy, ES<:EvolutionStrategy, SS<:ScalingStrategy} <: QDAlgorithm
...
function Base.show(io::IO, a::CFCIQMC)
    print(io, "CFCIQMC($(a.time_step_strategy), $(a.evolution_strategy), $(a.scaling_strategy))")
end
...
function advance!(report, state::QDReplicaState, s_state::PECSingleState)
    ...
    @unpack v, w, Hw, Hw_new, x, wm, id, current_scale = s_state
    ...
    if scaling_strategy isa DynamicScaling
        ...
        scale!(v, scaling_strategy.target_walkers/walkers_prev)
    ...
    @pack! s_state = v, w, Hw, Hw_new, x, wm, current_scale
...
function advance!(report, state::QDReplicaState, s_state::RKSingleState)
    ...
    @unpack v,w,x,wm,u1,u2,id,damping,current_scale = s_state
    @unpack time_step_parameters, shift, hamiltonian, time_step_strategy,
        reporting_strategy, algorithm = state
    @unpack time_step = time_step_parameters
    @unpack scaling_strategy = algorithm
    ...
    @pack! s_state = v,w,x,wm,u1,u2,id,current_scale
